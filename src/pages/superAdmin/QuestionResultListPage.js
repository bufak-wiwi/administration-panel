...
class QuestionResultListPage extends React.Component {
    ...
    renderOpenQuestionCard(question) {
        ...
        return (
            <Card key={question.questionID} style={{ marginBottom: 10 }}>
                ...
                <Col>
                    <Link to={`/questionresult/${question.questionID}`}>
                        <Button block>{ isUserAllowedToVote() ? "zu den Ergebnissen" : "Details anzeigen"}</Button>
                    </Link>
                </Col>
            </Card>
        )
...
const mapStateToProps = (state) => {
  ...
  export default connect(mapStateToProps, mapDispatchToProps)(QuestionResultListPage);