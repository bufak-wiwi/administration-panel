  ...
  return (
    <FormControl fullWidth variant="outlined">
      <InputLabel htmlFor={confirm ? 'password-confirm' : 'password'}>
        {confirm ? 'Passwort wiederholen' : isToken ? 'Token' : 'Passwort'}
      </InputLabel>
      <OutlinedInput
        ...
        error={isError()}
        disabled={disabled}
        label={confirm ? 'Passwort wiederholen' : isToken ? 'Token' : 'Passwort'}
        endAdornment={
      ...
      {password !== '' && !confirm && !isValidPassword(password) && (
        <FormHelperText error variant="outlined">
          {isToken ? 'Token' : 'Passwort'} muss zwischen 8 und 64 Zeichen lang sein
        </FormHelperText>
      )}
